...
1. Update memory files (`memory/` folder in repo root) with anything new worth remembering
...
- **Editor tap-to-edit:** `TextInput` is always `editable={true}`. `onFocus` → `setIsEditing(true)` (shows Done button, hides controls). `KeyboardAvoidingView behavior="padding"` wraps the content area — automatically lifts content when keyboard appears, so Done button stays above keyboard at `bottom: 16`. No manual keyboard height tracking. `onScrollBeginDrag` calls `Keyboard.dismiss()` when not in edit mode to prevent accidental keyboard during scroll. `scrollYRef` tracks scroll position via `onScroll` (scrollEventThrottle 16).
...
- **Editor app bar:** `position: absolute, top: 0` on screen, height `insets.top + 80`. BlurView fills the bar — blur extends behind the status bar (standard iOS pattern). Content rows use `paddingTop: insets.top` so Orra./Edit. sit below the status bar. Hairline border at bottom. Top scrim removed — app bar handles the cutoff. Title color `#8E8E93` for readability against blur. ScrollView is normal flow (SafeAreaView handles top inset), `paddingTop: 96` (80 bar + 16 gap) pushes text below the bar.
...
| 15 | Player stuck on white screen after natural end | `withTiming` callback (`finished`) silently drops under certain conditions — `router.back()` never fires. Fix: drive navigation with a plain `setTimeout(router.back, 1300)` instead. Animation still plays visually; timer is the reliable exit. |
| 16 | Absolute app bar overlaps status bar in editor | `position: absolute` children inside `SafeAreaView` ignore its padding — they start at y=0 of the screen. App bar must have `height: insets.top + 80` to cover the status bar area, and content rows need `paddingTop: insets.top` to sit below it. Normal-flow children (ScrollView) correctly respect SafeAreaView padding — do not double-add `insets.top` to their padding. |
| 17 | Tap to edit: cursor jumps to end, keyboard fires on scroll | `editable={isEditing}` + `onPressIn` failed: `onPressIn` doesn't fire on non-editable TextInput on iOS, requiring two taps. Manual `keyboardWillShow` listener + `marginBottom` didn't reliably keep cursor visible. Fix: `editable={true}` always + `onFocus` to enter edit mode + `KeyboardAvoidingView behavior="padding"` to handle keyboard offset + `Keyboard.dismiss()` in `onScrollBeginDrag` when not editing. |
| 18 | Options menu renders dark in light mode when system auto-dark is scheduled | `tint="systemMaterial"` follows the iOS system theme, not the app's own `darkMode` state. When the phone's scheduled dark mode activates, the blur renders dark even though the app is in light mode. Fix: always use explicit `tint="light"/"dark"` driven by app `darkMode`, with our own `rgba` background. Never use `systemMaterial`/`systemMaterialDark`. |
| 19 | Done button hidden behind keyboard (position:absolute ignores KAV padding) | `KeyboardAvoidingView behavior="padding"` only adjusts flow-layout children — `position:absolute` children anchor to the outer frame and don't move. Fix: keep KAV for scroll content, but separately track keyboard height via `keyboardWillShow`/`keyboardWillHide` listeners and set `bottom: keyboardHeight + 16` on the Done button. |
| 20 | Keyboard pops up / edit mode triggers accidentally while scrolling | `editable={true}` always meant any touch on the TextInput during a scroll could focus it. `Keyboard.dismiss()` in `onScrollBeginDrag` was a band-aid (fires too late). Fix: `editable={isEditing}` + a transparent `Pressable` overlay covering the TextInput when not editing. Tap the overlay → `setIsEditing(true)` + `setTimeout focus()`. Scroll gestures pass through to ScrollView naturally. `onScrollBeginDrag` now dismisses keyboard when IS editing (so keyboard hides while reading). |
